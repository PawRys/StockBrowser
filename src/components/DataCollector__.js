...
export function findProductErrors(data) {
	...
	for (let product of data) {
		...
		if (!!errors.length) {
			///////////////////////////////// ↓ replace duplicates
			const appendError = product.group.replace(/ ERROR/gi, '') + ' ERROR';
			Object.assign(product, { errors: errors, group: appendError });
		} else {
			Object.assign(product, { errors: [], group: product.group.replace(/ ERROR/gi, '') });
		}
		result.push(product);
	}
	return result;
